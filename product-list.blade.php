...
<body class="bg-light-bg text-gray-800 font-sans antialiased">
    ...
    <main class="min-h-screen pt-24 px-4 md:px-8 lg:px-16">
        ...
        <section class="flex flex-col items-center justify-center text-center py-24 bg-tertiary rounded-xl shadow-lg">
            <h1 class="text-5xl md:text-6xl font-bold font-serif text-primary animate-fadeIn" style="animation-delay: 0.2s;">
                @if(isset($product))
                    {{ $product->name }}
                @else
                    Our Products
            ...
            <p class="mt-4 text-xl text-primary font-sans animate-fadeIn" style="animation-delay: 0.4s;">
                @if(isset($product))
                    {{ $product->category_names ?? 'HANDMADE BY TRIBAL ARTISANS' }}
                @else
        ...
        <section id="products-list-page" class="py-16 page-section {{ !isset($product) ? 'active' : '' }}">
                            ...
                            <div class="space-y-2 text-primary">
                                ...
                                <label class="flex items-center">
                                    <input type="checkbox" name="category" value="{{ $category->name }}" 
                                           ...
                                           {{ isset($selectedCategory) && $selectedCategory === $category->name ? 'checked' : '' }}> 
                                    {{ $category->name }}
                                </label>
                                @endforeach
                            </div>
                ...
                <div class="bg-white rounded-xl shadow-lg p-8 grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12 max-w-full overflow-hidden">
                    ...
                    <div class="flex flex-col justify-center min-w-0">
                        ...
                        <div class="flex flex-col sm:flex-row gap-4">
                            ...
                            <a href="#" id="buy-now-btn" class="bg-secondary text-light-bg font-semibold py-3 px-6 rounded-full shadow-md hover:bg-primary transition-colors duration-300 transform hover:scale-105 animate-slideInFromRight text-center" style="animation-delay: 0.5s;">
                                Buy Now
                            </a>
                        </div>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Debug: Show products in a visible way
            if (products.length === 0) {
                console.log('No products found, showing debug message');
                document.getElementById('product-list').innerHTML = '<div class="col-span-full text-center text-red-500 p-8"><h3>DEBUG: No products found</h3><p>Check console and server logs for details</p></div>';
            } else {
                console.log('Products found, calling displayProducts');
                // Call displayProducts immediately to show the products
            ...
            function displayProducts(productArray) {
                console.log('displayProducts called with:', productArray);
                ...
                try {
                    ...
                    console.log('Displaying', productArray.length, 'products');
                    ...
                    productArray.forEach((product, index) => {
                        ...
                        card.innerHTML = `
                            <div class="w-full h-64 overflow-hidden bg-gray-50 flex items-center justify-center">
                                <img src="${product.image}" alt="${product.name}" class="max-w-full max-h-full object-contain" onerror="console.log('Image failed to load:', this.src); this.src='{{ url('public/images/product/placeholder.jpg') }}'">
                            </div>
                            <div class="p-4 text-center">
                                <h3 class="text-lg font-semibold text-primary font-serif">${product.name}</h3>
                                <p class="text-md font-bold text-secondary mt-1">${product.category}</p>
                            </div>
                        ...
                        console.log(`Card ${index + 1} added to DOM`);
                    });
                    
                    console.log('All products displayed successfully');
                } catch (error) {
                    console.error("Failed to display products:", error);
                    productList.innerHTML = '<div class="col-span-full text-center text-red-500 p-8"><h3>Error displaying products</h3><p>' + error.message + '</p></div>';
                }
            }
            ...
            // Initial load
            updateCartCount();
            // displayProducts is now called above when products are loaded
            
            // Auto-apply filters if a category is pre-selected
            const hasPreSelectedCategory = Array.from(categoryCheckboxes).some(cb => cb.checked);
